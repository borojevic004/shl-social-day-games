const SHLMatchmaker = (() => {
  ...
  function withDisplayScores(matches) {
    const topScore = matches[0]?.rawMatchScore || 0;
    let previousScore = 101;

    return matches.map((job, index) => {
      const displayScore = Math.min(
        previousScore - (index === 0 ? 0 : Math.floor(Math.random() * 4) + 3),
        SHLMatching.displayScoreFromRank(job.rawMatchScore, index, topScore)
      );
  ...
  function buildBalancedMatches() {
    const candidateLimit = Math.min(jobs.length, 30);
    const candidates = SHLMatching.getBestMatches(
      jobs,
      interests,
    ...
    const selectedMatches = [bestJob];
    const remainingCandidates = candidates.slice(1);

    while (selectedMatches.length < RESULT_LIMIT && remainingCandidates.length) {
      const previousMatch = selectedMatches[selectedMatches.length - 1];
      const nextIndex = remainingCandidates
        .map((job, index) => ({
          index,
          score:
            job.rawMatchScore +
            SHLMatching.calculateJobSimilarity(bestJob, job) * 18 +
            SHLMatching.calculateJobSimilarity(previousMatch, job) * 8 -
            Math.max(0, selectedMatches.filter((match) => match.group === job.group).length - 1) * 6 +
            Math.random() * 2
        }))
        .sort((a, b) => b.score - a.score)[0].index;

      selectedMatches.push(remainingCandidates.splice(nextIndex, 1)[0]);
    }

    return withDisplayScores(selectedMatches);
  }
  ...
  function showResults() {
    ...
    state.bestMatches = buildBalancedMatches();
    ...
    renderRankingModal();
    SHLConfetti.start(MATCH_CONFETTI);
    popCard();
  }
